function mainsys=gen_windfarm_no_taylor(mdlfile,path,farm,wind)
...
mainsys=new_system(mdlname,'model','libaeolus/Farm Template - No Taylor');
...
%Add farmlayout to Windfield block
temp_p.farm.x = farmx;
temp_p.farm.y = farmy;
...
h=get_param([wfname '/Deficit'],'PortHandles');
set_param(h.Outport(1),'DataLogging','on');
...
for i=1:length(farm.turbines)
    ...
    wtsp(i).block=add_block(['libwind/Wind speed - No Taylor'],[mdlname '/Wind Field/Wind Speed' int2str(i)]);
    %Set block position
    ...
    %Input wind speed
    add_line(wfname,'Deficit/1',['Wind Speed' int2str(i) '/1'],'Autorouting','on')
