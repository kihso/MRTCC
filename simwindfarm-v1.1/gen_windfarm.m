...
mainsys=new_system(mdlname,'model','libaeolus/Farm Template - Taylor');
...
for i=1:length(farm.turbines)
    ...
    set_param(wtsp(i).block,'Position',[wsp(1)+x+50 wsp(2)+y,wsp(3)+x+50 wsp(4)+y]);
    %Unlink block for modification
    ...
    for j=1:length(delays(i,:))
        ...
        if(delays(i,j)>0)
            %CT delay block
            ctd=add_block('simulink/Discrete/Integer Delay',[wfname '/Wake' int2str(i) '/aDelay' int2str(j)], ...
                        'MakeNameUnique','on' ,...
            ...
            add_block('simulink/Sinks/Terminator',[wfname '/Wake' int2str(i) '/TermAde' int2str(j)], ...
                        'MakeNameUnique','on');
